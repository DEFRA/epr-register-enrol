...
| Where | Key | Notes |
|---|---|---|
| `management-fe` | `SAMPLING_PLAN_S3_BUCKET` env var (`fileStorage.samplingPlanBucket`, default `epr-register-enrol-sampling-plans`) | Only a **fallback** — real downloads use the `s3Bucket` stored on the file record, not this default. Matters mainly for legacy records. |
...
| Platform/infra (not app config) | S3 `GetObject` IAM permission | `management-fe`'s IAM role needs read access to whatever bucket(s) sampling-plan and BES-evidence files actually land in for that environment — check against `epr-register-enrol-backend`'s `CdpUploader.SamplingPlanBucket` / `BesEvidenceBucket` / `GenericFilesBucket` config for that same environment, since that's what actually determines the bucket names stored on file records. |

⚠️ **Known default mismatch to watch for:** `management-fe`'s fallback default is `epr-register-enrol-sampling-plans`, but `epr-register-enrol-backend`'s default (`appsettings.json`) is just `sampling-plans` (no prefix). These only need to agree if you're relying on the fallback (i.e. for records missing `s3Key`) — but worth confirming actual per-environment bucket names with whoever owns the Terraform/CDP provisioning, since a mismatch there means a real, silent download failure.
